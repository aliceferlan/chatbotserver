...
function removeJsonMarkdown(input: string): string {
    try {
        ...
        console.error("JSONマークダウンの解析中にエラーが発生しました:", e);
        return input; // エラーが発生した場合は元の入力を返す
    }
}

export async function sendRequestToAI(input: ImageData): Promise<RecieptResponse | RecieptErrorResponse> {
    try {
        // 最初にgemini-1.5-flashモデルを試す
        const response = await send(input, "gemini-1.5-flash");

        // JSONを解析
        const cleanedResponse = removeJsonMarkdown(response);
        ...
        try {
            ...
            if ('error' in responseJson) {
                return responseJson as RecieptErrorResponse;
            }

            return responseJson as RecieptResponse;
        } catch (parseError) {
            console.error("JSONの解析に失敗しました:", parseError);
            console.log("解析に失敗したテキスト:", cleanedResponse);

            // バックアップとしてgemini-1.5-proモデルを試す
            console.log("バックアップモデルを使用して再試行します...");
            const reResponse = await send(input, "gemini-1.5-pro");
            const reCleanedResponse = removeJsonMarkdown(reResponse);

            try {
                return JSON.parse(reCleanedResponse);
            } catch (reParseError) {
                throw new Error("両方のモデルでJSONの解析に失敗しました");
            }
        }
    } catch (error) {
        ...
        return {
            ...
            contentType: "エラー",
            suggestion: error instanceof Error ? error.message : "再度お試しください"
        };
    }
}
